...
function light() {
  ...
  let pageBanner = async ()=> {
         await fetch(url)
          ...
          movieBanner(Recent,i)
          displayBanner(Recent,i)
    ...
    function displayBanner(Recent,i) {
      bannerDetails1.innerHTML += 
          `<div>
            <h1>${Recent[i].title}</h1>
                <div class="textContainer">
                        <span>
  ...
  function recentMovies(recentMovie) {
    ...
    for (let index = 0; index < recentMovie.length; index++) {
      ull.innerHTML += ` <li class='imgList'>
          <img src="${recentMovie[index].Image}" alt="" class="image">
          <div id='overlay'>
            <div>
  ...
  function PopularMovies(popularMovie) {
    ...
    for (let index = 0; index < popularMovie.length; index++) {
      ulll.innerHTML += ` <li class='imgList'>
          <img src="${popularMovie[index].Image}" alt="" class="image">
          <div id='overlay'>
            <div>
              <button class='btn' id='button' onclick="getInfo(${index})">Movie Details</button>
              </div>
            </div>
  </li>      
   `;
